...
def precess_image_clip(image_path ,duration , json ,width, height):
    ...
    image_clip = novel_tools.resize_image(image_path, width , height).set_duration(duration)

    if video_effects == "放大":
          image_clip = novel_tools.resize_image(image_path, width , height).set_duration(duration)
    if video_effects == "左":
          image_clip = novel_tools.resize_image(image_path, width+ 25 * duration, height).set_duration(duration)
    if video_effects == "左放大":
          image_clip = novel_tools.resize_image(image_path, width + 25 * (duration-2), height).set_duration(duration)
    if video_effects == "右放大":
          image_clip = novel_tools.resize_image(image_path, width + 25 * (duration-2), height).set_duration(
            duration).set_position((-(25 * (duration-2)), 0))
    if video_effects == "右":
          image_clip = novel_tools.resize_image(image_path, width + 25 * duration, height).set_duration(
            duration).set_position((-(25 * duration), 0))
    if video_effects == "上":
        image_clip = novel_tools.resize_image(image_path, width, height + 25 * duration).set_duration(duration).set_position((0,0))
    if video_effects == "下":
        image_clip = novel_tools.resize_image(image_path, width, height + 25 * duration).set_duration(
            duration).set_position((0, -25 * duration))
    if video_effects == "右上":
        image_clip = novel_tools.resize_image(image_path, width + 25 * duration, height + 25 * duration).set_duration(
            8).set_position((-25 * duration, 0))
    if video_effects == "左上":
        image_clip = novel_tools.resize_image(image_path, width + 25 * duration, height + 25 * duration).set_duration(
            8).set_position((0, 0))
    ...
    if video_filters not in [None, ""]:
        for object in video_filters:
            apply_shake_partial= partial(apply_shake_effect, begin=object["begin"], end=object["end"], type=object["name"])
...
def vibration_weak_effect(image, intensity=5):
    ...
    matrix = np.float32([[1, 0, dx], [0, 1, dy]])
    # 应用仿射变换
    shaken_image = cv2.warpAffine(image, matrix, (width+25, height+25))
    return shaken_image
...
def shift_frame_left_top(image, t, end,shift_pixels=50):
    ...
    # 构建仿射变换矩阵（放大）
    M_zoom = np.float32([[1, 0, -((end - 2) * 25)],
                         [0, 1, 0]])
...
def apply_shake_effect(get_frame, t, begin, end, type):
    ...
    if type == "震动" and (int(t / 0.15) % 2) == 0 and begin <= t < end:
        return vibration_effect(frame, 25)
